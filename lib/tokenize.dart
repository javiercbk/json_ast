...
enum _StringState {
  _START_, START_QUOTE_OR_CHAR, ESCAPE
}
...
Token parseKeyword(String input, int index, int line, int column) {
  final entries = keywordTokensMap.entries;
    ...
    final keyLen = entry.key.length;
		if (input.substring(index, entry.key.length) == entry.key) {
      return new Token(entry.value, line, column + keyLen, index + keyLen, entry.key);
		}
	}
		...
		switch (state) {
			...
			case _StringState.START_QUOTE_OR_CHAR: {
				if (char == '\\') {
					...
					index ++;
					return new Token(TokenType.STRING, line, column + index - startIndex, index, input.substring(startIndex, index));
				} else {
					// buffer.write(char);
					index ++;
				}
...
Token parseNumber(String input, int index, int line, int column) {
	...
	if (passedValueIndex > 0) {
		return new Token(TokenType.NUMBER, line, column + passedValueIndex - startIndex, passedValueIndex, input.substring(startIndex, passedValueIndex));
	}
...
List<_tokenParser> _parsers = [parseChar, parseKeyword, parseString, parseNumber];
...
List<Token> tokenize(String input, Settings settings) {
	...
	while (index < input.length) {
		...
		if (token != null) {
			token.loc = Location.create(line, column, index, token.line, token.column, token.index, settings.source);
			tokens.add(token);
			index = token.index;
			line = token.line;
			column = token.column;
			...
			final msg = unexpectedSymbol(substring(input, index, index + 1), [settings.source, line, column]);
