...
enum _ObjectState {
  _START_, OPEN_OBJECT, PROPERTY, COMMA
}
...
enum _ArrayState {
  _START_, OPEN_ARRAY, VALUE, COMMA
}

JSONASTException errorEof(String input, List<dynamic> tokenList, Settings settings) {
	final Loc loc = tokenList.length > 0
		? tokenList[tokenList.length - 1].loc.end
		: new Loc(line: 1, column: 1);

	return new JSONASTException(unexpectedEnd(),
		input,
		settings.source,
		loc.line,
		loc.column
...
String parseString(String string) {
	...
	for (int i = 0; i < string.length; i ++) {
		...
		if (char == '\\') {
			...
			if (nextChar == 'u') {
        result.write(parseHexEscape(string.substring(i + 1, 4)));
				i += 4;
...
ValueIndex<ObjectNode> parseObject(input, List<Token> tokenList, int index, Settings settings) {
		...
		switch (state) {
          ...
          final msg = unexpectedToken(
            ...
            token.loc.start.column
					...
					throw new JSONASTException(msg, input, settings.source, token.loc.start.line, token.loc.start.column);
				}
				break;
			}
...
ValueIndex<PropertyNode> parseProperty(String input, List<Token> tokenList, int index, Settings settings) {
	...
	while (index < tokenList.length) {
		...
		switch (state) {
          ...
          final value = parseString(input.substring(token.loc.start.offset + 1, token.loc.end.offset - 1));
...
ValueIndex<ArrayNode> parseArray(String input, List<Token> tokenList, int index, Settings settings) {
...
ValueIndex<LiteralNode> parseLiteral(String input, List<Token> tokenList, int index, Settings settings) {
	...
	switch (token.type) {
		case TokenType.STRING: {
			value = parseString(input.substring(token.loc.start.offset + 1, token.loc.end.offset - 1));
			break;
		}
...
typedef ValueIndex _parserFun(String input, List<Token> tokenList, int index, Settings settings);

List<_parserFun> _parsersList = [parseLiteral, parseObject, parseArray];

ValueIndex _findValueIndex(String input, List<Token> tokenList, int index, Settings settings) {
...
ValueIndex _parseValue(String input, List<Token> tokenList, int index, Settings settings) {
...
ValueIndex parse(input, settings) {
